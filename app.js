...
function resize() {
  ...
  if (hasFrames) drawFrame(Math.round(currentFrame));
}
...
function loadFrame(index) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      if (img.decode) {
        img.decode().then(() => { frames[index] = img; resolve(true); }).catch(() => { frames[index] = img; resolve(true); });
      } else {
        frames[index] = img;
        resolve(true);
      }
...
async function loadAllFrames() {
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      const ok = await loadFrame(idx);
      if (ok) loaded++; else failed++;
      const total = loaded + failed;
      const pct = Math.floor((total / TOTAL_FRAMES) * 100);
      loaderBar.style.width = pct + '%';
      loaderPct.textContent = pct;
      loaderCap.textContent = msgs[Math.min(msgs.length - 1, Math.floor(pct / 22))];
      // If first 10 frames all fail, abort — no frames directory
      if (failed > 10 && loaded === 0) return;
    }
  }
...
window.addEventListener('scroll', () => {
  ...
  if (sy > 80) {
    navbar.style.background = 'rgba(5, 8, 14, 0.95)';
    navbar.style.borderColor = 'rgba(0,163,255,.3)';
  } else {
    navbar.style.background = 'rgba(5, 8, 14, 0.85)';
    navbar.style.borderColor = 'rgba(0,163,255,.2)';
  }
}, { passive: true });

/* ── Section observer (nav-link offset: idx-1) ── */
...
function initEffects() {
  // Card glow
  document.querySelectorAll('.service-card, .result-card, .package-card, .process-step').forEach(c => {
